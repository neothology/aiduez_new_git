...
class TabularWorkbook:
    ...
    def save_current_work(self):
        ...
        # save current work -(4) AI모델 학습(모델관련 내용은 '학습'할 때 저장되므로 여기서는 modeling options 만 저장)
        self.app_context.tabular_ai_training__modeling_options.save_config(self.current_work_state_dir)
    ...
    def load_existing_work(self, work_name):
        ...
        # load new work -(1) 데이터 분석
        if self.app_context.current_workflow_stage == 'tabular_data_analyze':
            pass
        # load new work -(2) 데이터 가공
        elif self.app_context.current_workflow_stage == 'tabular_data_processing':
            pass
        # load new work -(3) AI모델 학습
        elif self.app_context.current_workflow_stage == 'tabular_ai_training':
            self.app_context.tabular_ai_training.change_data()
